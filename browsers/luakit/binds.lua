...
add_binds("normal", {
    ...
    key({}, "_", "Copy Lastpass password",
        function(w) 
            local uri = string.gsub(w.view.uri or "", " ", "%%20")
            os.execute("~/.config/luakit/lastpass.sh " .. uri)
        end),
    ...
    key({}, "d", "Close current tab (or `[count]` tabs).", function (w, m) for i=1,m.count do if w.tabs:count() > 1 then w:close_tab() else w:close_win() end end end, {count=1}),
